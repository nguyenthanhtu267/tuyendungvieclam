...
async function bootstrap() {
  ...
  app.enableCors({ origin: resolveCorsOrigins(process.env.CORS_ORIGIN), credentials: true });
...
bootstrap().catch((err) => {
  ...
  logger.error(`Không thể khởi động server: ${err instanceof Error ? err.message : err}`);
  process.exit(1);
});
